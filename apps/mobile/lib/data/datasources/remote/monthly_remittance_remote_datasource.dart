...
abstract class MonthlyRemittanceRemoteDataSource {
  Future<RemittanceCalculationRequestModel> getAllCalculatedRemittanceHistory(
    String userId,
  );
  Future<RemittanceCalculationResponseModel> calculateRemittanceAmount(
    RemittanceCalculationRequestModel monthlyRemittance,
    String? userId,
  );
}

class MonthlyRemittanceRemoteDataSourceImpl
    implements MonthlyRemittanceRemoteDataSource {
  MonthlyRemittanceRemoteDataSourceImpl({required this.dio});
  ...
  Future<RemittanceCalculationResponseModel> calculateRemittanceAmount(
    ...
    try {
      ...
      if (result.statusCode == 200) {
        return RemittanceCalculationResponseModel.fromJson(
